...
switch ($table) {
	...
	break;
	case 'departments':
		$query = "SELECT departments.dept_no, departments.dept_name, COUNT(dept_emp.dept_no) AS total_employees FROM departments JOIN dept_emp ON departments.dept_no = dept_emp.dept_no GROUP BY departments.dept_no";
	break;
	case 'titles':
